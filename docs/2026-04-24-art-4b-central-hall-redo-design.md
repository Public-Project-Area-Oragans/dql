...
현 배포(`/dql/`) 중앙 홀의 시각 붕괴를 조립식 컴포지션으로 교체. 풀스크린 opaque bg + 분리 가능한 transparent overlay 12종으로 art-8(폴리시) 단계에서 개별 애니메이션·교체가 가능하도록 한다.
...
- entrance_arch 는 각 도어 앵커 기준 `arch.center == door.center`, size 동일 (도어 뒤 프레임)
- 라벨 TextComponent: `anchor = topCenter`, `y = door.bottom + 4`

### 2.3 에셋 로드 파이프라인

- `AssetPreloader.loadMainHallAssets()` 가 10 PNG 병렬 로드
- 개별 로드 실패 시 `fallbackRectangleColor` (art-2b 에서 검증된 패턴) 로 대체
- 전체 로드 완료 이벤트 → `CentralHallScene.assemble()` 호출
...
| 파일 | 변경 |
|---|---|
| `lib/game/scenes/central_hall_scene.dart` | 3층 parallax 로드·렌더 제거. `assemble()` 이 base → pillar × 2 → entrance_arch × 4 → chandelier + compass → door × 4 → label 순서로 add |
| `lib/game/assets/asset_ids.dart` | 신규 9 상수 추가. 기존 `env_mainhall_bg_{far,mid,near}` 는 deprecated 주석 후 씬 참조 제거 (파일 삭제는 art-5 일괄) |
| `lib/game/assets/asset_preloader.dart` | `loadMainHallAssets()` 의 로드 대상 갱신. fallback 색 매핑 유지 |
...
| `.tmp/check_opacity.py` | PIL 기반 corner alpha 검증. `.tmp/` 는 gitignore 가정, repo 포함 X |

### 4.3 에셋 파일

| 경로 | 비고 |
|---|---|
| `assets/sprites/env/env_mainhall_base.png` | 신규 |
| `assets/sprites/deco/deco_pillar_left.png` | 신규 (right 는 mirror 로 재사용) |
| `assets/sprites/deco/deco_entrance_arch_backend.png` | 신규 |
| `assets/sprites/deco/deco_entrance_arch_database.png` | 신규 |
| `assets/sprites/deco/deco_entrance_arch_frontend.png` | 신규 |
| `assets/sprites/deco/deco_entrance_arch_architecture.png` | 신규 |
| `assets/sprites/deco/deco_chandelier_central.png` | 신규 |
| `assets/sprites/deco/deco_compass_rose_floor.png` | 신규 |
| `assets/sprites/obj/obj_door_architecture.png` | 덮어쓰기 (재생성) |
